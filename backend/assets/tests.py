...
class TestAssetSharedInventoryContract:
    def test_peer_can_get_asset_by_pk(
    ...
    def test_peer_can_get_asset_by_guid(
        self, authenticated_client, client_as_user_two, user, user_two
    ):
        create = authenticated_client.post(
            "/api/v1/assets/", {"name": "Peer GUID Asset"}
        )
    ...
    def test_peer_can_patch_asset_created_by_other(
        ...
        aid = create.data["id"]
        r = client_as_user_two.patch(
            f"/api/v1/assets/{aid}/", {"name": "After"}, format="json"
        )
        assert r.status_code == 200
        assert r.data["name"] == "After"
        assert Asset.objects.get(pk=aid).name == "After"

    def test_peer_sees_others_asset_in_list(
        self, authenticated_client, client_as_user_two, user, user_two
    ):
...
class TestAssetAPI:
    ...
    def test_create_asset_requires_display_name_with_guid(
    ...
    def test_soft_delete_requires_admin(self, authenticated_client, user):
        ...
        response = authenticated_client.post(
            f"/api/v1/assets/{asset_id}/delete/",
            {
                "delete_reason": "lost",
            },
        )
    ...
    def test_create_asset_strict_custom_fields_missing_key(
        self, authenticated_client, user
    ):
    ...
    def test_create_asset_optional_decimal_json_null_ok(
        self, authenticated_client, user
    ):
    ...
    def test_create_asset_rejects_custom_fields_when_none_configured(
        self, authenticated_client, user
    ):
        response = authenticated_client.post(
            "/api/v1/assets/",
